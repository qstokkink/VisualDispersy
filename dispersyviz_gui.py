...
class MainWindow(Gtk.Window):
    ...
    def __init__(self):
        ...
        self.treeview.append_column(Gtk.TreeViewColumn("Found classes:", Gtk.CellRendererText(), text=0))
        ...
        grid.attach_next_to(numpeers_label, scrollable, Gtk.PositionType.BOTTOM, 2, 1)
        grid.attach_next_to(numpeers_entry, numpeers_label, Gtk.PositionType.BOTTOM, 1, 1)
        grid.attach_next_to(nummessages_label, numpeers_entry, Gtk.PositionType.BOTTOM, 2, 1)
        grid.attach_next_to(self.nummessages_entry, nummessages_label, Gtk.PositionType.BOTTOM, 2, 1)
    ...
    def _validate_nummessages_script(self):
        ...
        for i in range(1, int(self.numpeers)+1):
            try:
                msgs = eval(self.nummessages_entry.get_text(), {}, {'peerid': i})
    ...
    def on_run(self, widget):
        ...
        if selection[1]:            
            ...
            if not uinput:
                self._blocking_dialog("ERROR", Gtk.MessageType.ERROR, "Your number of messages definition is not a valid number or python line.")
                return
            ...
            try:
                ...
                if sys.exc_info()[2]:
                    # TODO Remove our own statement from this traceback, user will have no idea what __import__(name) means
                    self._blocking_dialog("ERROR", Gtk.MessageType.ERROR, "Import of %s.py failed.\n\n%s" % (name, traceback.format_exc(sys.exc_info()[2])))
                else:
                    self._blocking_dialog("ERROR", Gtk.MessageType.ERROR, "Import of %s.py failed with error:\n\n%s: %s" % (name, sys.exc_info()[0], sys.exc_info()[1]))
                return
            sys.path.pop(0)

            # Find a main function
            if not "main" in [str(member) for member in dir(module)]:
                self._blocking_dialog("ERROR", Gtk.MessageType.ERROR, "Could not find a main() function in %s.py." % (name))
                return
            ...
            if len(inspect.getargspec(main_function)[0]) != 5:
                self._blocking_dialog("ERROR", Gtk.MessageType.ERROR, "The main() function of %s.py needs to accept 4 arguments (peerid, total peers, starting messages, total messages, visual dispersy server port)." % (name))
                return
            ...
            tries_left = 3
            while tries_left > 0:
                p_visualserver = subprocess.Popen(['python', '-u', 'dispersyviz/visualserver.py', str(p_vs_port)], cwd = os.path.join(os.path.dirname(os.path.abspath(__file__))), stdout=subprocess.PIPE, bufsize=1)
            ...
            if p_visualserver.returncode:
                self._blocking_dialog("ERROR", Gtk.MessageType.ERROR, "Well this is embarrassing. The VisualServer object crashed before the experiment even began. Contact a programmer.")
                return

            # Launch processes once server has started
            children = [subprocess.Popen(['python', '-c', "import os,sys;sys.path.insert(0, '%s');peerid=%s;nummessages=%s;numpeers=%s;totalmessages=%s;getattr(__import__('%s'), 'main')(peerid,numpeers,nummessages,totalmessages,%s)" % (os.path.join(os.path.dirname(os.path.abspath(__file__)), "experiments"), str(peerid), nummessages, self.numpeers, totalmessages, name, str(p_vs_port))], cwd = os.path.join(os.path.dirname(os.path.abspath(__file__)))) for peerid in range(1, int(self.numpeers)+1)]

            self._blocking_dialog("RUNNING EXPERIMENT", Gtk.MessageType.INFO, "Busy conducting experiment. Close this window to terminate all running processes.")
